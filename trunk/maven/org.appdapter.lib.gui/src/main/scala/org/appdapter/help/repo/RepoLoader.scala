...
import org.appdapter.core.store.SpecialRepoLoader
import org.appdapter.core.matdat.FabricLoaderRepo
...
object RepoLoader extends BasicDebugger {
  ...
  def makeSheetRepo(spec: RepoSpec, dirModel: Model, fileModelCLs: java.util.List[ClassLoader] = null, dirGraphID: Ident = null): SheetRepo = {
    ...
    val shRepo = new FabricLoaderRepo(spec, specURI, myDebugName, dirModel, fileModelCLs)
...
class RepoLoader extends SpecialRepoLoader {
}
