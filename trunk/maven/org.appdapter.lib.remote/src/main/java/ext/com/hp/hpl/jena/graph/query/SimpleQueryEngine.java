...
public class SimpleQueryEngine {
	...
	*/
	@Deprecated public SimpleQueryEngine(NamedTripleBunches triples, TripleSorter ts, ExpressionSet constraint) {
	...
	private void addStages(ArrayList<Stage> stages, NamedGraphMap arguments, Mapping map) {
		...
		while (it2.hasNext()) {
			...
			Stage next = SimpleQueryHandler.findOrCreate(g).patternStage(map, constraint, nodes);
			stages.add(next);
		}
	}
