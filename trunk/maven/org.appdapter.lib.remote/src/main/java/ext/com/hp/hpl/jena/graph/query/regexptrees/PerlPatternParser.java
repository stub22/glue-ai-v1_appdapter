...
public class PerlPatternParser {
	...
	*/
	public static final String wordChars = digits + "abcdefghijklmnopqrstuvwxyz" + "_" + "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
	...
	*/
	public RegexpTree parseAtom() {
		if (pointer < limit) {
			...
			switch (ch) {
			case '.':
				return generator.getAnySingle();
			case '^':
				return generator.getStartOfLine();
			case '$':
				return generator.getEndOfLine();
			case '|':
				pointer -= 1;
			...
			case '?':
			case '{':
				throw new PerlPatternParser.SyntaxException("unbound quantifier " + ch);
			case ']':
			case '}':
			default:
	...
	*/
	protected RegexpTree parseParens() {
		...
		if (pointer < limit && toParse.charAt(pointer) == ')')
			pointer += 1;
		else
	...
	*/
	protected RegexpTree backReferenceOrOctalChar(char ch) {
		...
		return 0 < n && n <= matchPointsSeen ? generator.getBackReference(n) : generator.getText(numeric(chars, 8, index));
	}
	...
	*/
	public RegexpTree parseSeq() {
		...
		while (true) {
			...
			if (next.equals(generator.getNothing()))
				break;
			operands.add(next);
		}
