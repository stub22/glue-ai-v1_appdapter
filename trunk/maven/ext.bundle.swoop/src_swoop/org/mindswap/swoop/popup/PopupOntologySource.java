...
public class PopupOntologySource extends JFrame implements KeyListener, ActionListener, WindowListener, DocumentListener, MouseListener {
	...
	public int matchString(boolean forward, boolean showError) {
		...
		try {
			...
			while (currentMatch >= 0 && currentMatch < (doc.getLength() - findMatch.length())) {
				...
				String prevChar = "_";
				if (currentMatch >= 1)
					prevChar = doc.getText(currentMatch - 1, 1);
				String nextChar = "_";
				if (currentMatch + findMatch.length() < doc.getLength())
					nextChar = doc.getText(currentMatch + findMatch.length(), 1);
				int pcharValue = (int) prevChar.charAt(0);
				int ncharValue = (int) nextChar.charAt(0);
				boolean charsNotAlphabet = true;
				if (((ncharValue >= 65 && ncharValue <= 90) || (ncharValue >= 97 && ncharValue <= 122)) || ((pcharValue >= 65 && pcharValue <= 90) || (pcharValue >= 97 && pcharValue <= 122)))
				...
				if ((matchWord && check.equals(findMatch) && charsNotAlphabet) || (!matchWord && check.indexOf(findMatch) >= 0)) {
					matchFound = true;
			...
			if (matchFound) {
				...
				if (showError)
					JOptionPane.showMessageDialog(this, "No more matches found!", "Find Error", JOptionPane.ERROR_MESSAGE);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == findMenu) {
			...
			String findNewMatch = JOptionPane.showInputDialog(null, new Object[] { "Specify Search String:", caseChk, wordChk }, "Find", JOptionPane.PLAIN_MESSAGE);
