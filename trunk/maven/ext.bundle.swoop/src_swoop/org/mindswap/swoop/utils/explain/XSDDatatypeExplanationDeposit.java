...
public class XSDDatatypeExplanationDeposit extends Hashtable implements DatatypeExplanationDeposit {
	...
	private XSDDatatypeExplanationDeposit() throws URISyntaxException {
		...
		add(new DatatypeExplanation(new URI(name), name, "The string datatype represents character strings in XML. The value" + " space of string is the set of finite-length sequences of characters "
				+ "(as defined in [XML 1.0 (Second Edition)]) that match the Char production" + " from [XML 1.0 (Second Edition)]. A character is an atomic unit of "
				+ "communication; it is not further specified except to note that every " + "character has a corresponding Universal Character Set code point, which is" + " an integer.", uris,
				examples));

		name = XMLSchemaSimpleDatatypeVocabulary.XS + "boolean";
		String[] ex_boolean = { "true", "false", "1", "0" };
		add(new DatatypeExplanation(new URI(name), name, "boolean has the value space required to support the mathematical concept " + "of binary-valued logic: {true, false}.", uris, ex_boolean));

		name = XMLSchemaSimpleDatatypeVocabulary.XS + "decimal";
		String[] ex_decimals = { "-1.23", "12678967.543233", "+100000.00", "210" };
		add(new DatatypeExplanation(new URI(name), name, "decimal represents a subset of the real numbers, which can be represented"
				+ " by decimal numerals. The value space of decimal is the set of numbers that" + " can be obtained by multiplying an integer by a non-positive power of ten,"
				+ " i.e., expressible as i × 10^-n where i and n are integers and n >= 0." + " Precision is not reflected in this value space; the number 2.0 is not"
				+ " distinct from the number 2.00. The order-relation on decimal is the" + " order relation on real numbers, restricted to this subset.", uris, ex_decimals));

		name = XMLSchemaSimpleDatatypeVocabulary.XS + "float";
		String[] ex_float = { "-1E4", "1267.43233E12", "12.78e-2", "12", "-0", "0", "INF" };
		add(new DatatypeExplanation(
				new URI(name),
				name,
		...
		add(new DatatypeExplanation(
				new URI(name),
				name,
				"time represnts an instant of time that recurs every day. The value space of time is the space of time of day values as defined in § 5.3 of [ISO 8601]. Specifically, it is a set of zero-duration daily time instances.",
				uris, ex_time));
		...
		add(new DatatypeExplanation(
				new URI(name),
				name,
				"hexBinary represents arbitrary hex-encoded binary data. The value space of hexBinary is the set of finite-length sequences of binary octets. (Note that lower case [a-f] are not allowed)",
				uris, ex_hexBin));
		...
		add(new DatatypeExplanation(
				new URI(name),
				name,
				"TName represents XML Names. The value space of Name is the set of all strings which match the Name production of [XML 1.0 (Second Edition)]. The lexical space of Name is the set of all strings which match the Name production of [XML 1.0 (Second Edition)]. The base type of Name is token.",
				uris, null));
		...
		add(new DatatypeExplanation(
				new URI(name),
				name,
				"long is derived from integer by setting the value of maxInclusive to be 9223372036854775807 and minInclusive to be -9223372036854775808. The base type of long is integer.  3.3.16.1 Lexical representation",
				uris, ex_long));
		...
		add(new DatatypeExplanation(new URI(name), name, "short is derived from int by setting the value of maxInclusive to be 32767 and minInclusive to be -32768. The base type of short is int.",
				uris, ex_short));

		name = XMLSchemaSimpleDatatypeVocabulary.XS + "byte";
		String[] ex_byte = { "-1", "0", "126", "+100" };
		add(new DatatypeExplanation(new URI(name), name, "byte is derived from short by setting the value of maxInclusive to be 127 and minInclusive to be -128. The base type of byte is short.",
				uris, ex_byte));
		...
		add(new DatatypeExplanation(new URI(name), name, "unsignedShort is derived from unsignedInt by setting the value of maxInclusive to be 65535. The base type of unsignedShort is unsignedInt.",
				uris, ex_US));

		name = XMLSchemaSimpleDatatypeVocabulary.XS + "unsignedByte";
		String[] ex_UB = { "0", "126", "100" };
		add(new DatatypeExplanation(new URI(name), name, "unsignedByte is derived from unsignedShort by setting the value of maxInclusive to be 255. The base type of unsignedByte is unsignedShort.",
				uris, ex_UB));
