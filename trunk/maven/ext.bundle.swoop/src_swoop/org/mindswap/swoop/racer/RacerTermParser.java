...
public class RacerTermParser {
	...
	protected Object parseAll(Vector v) throws RacerIllegalConstruction {
		if (v.size() != 3)
			throw new RacerIllegalConstruction(v);
	...
	public Object parseConcept(String c) throws RacerIllegalConstruction {
		if (c.equalsIgnoreCase("top"))
			return buildTop();
		if (c.equalsIgnoreCase("bottom"))
			return buildBottom();
		if (c.charAt(0) == '|' && c.charAt(c.length() - 1) == '|')
			return buildLitteral();
		if (c.indexOf(' ') == -1 && c.indexOf('(') == -1 && c.indexOf(')') == -1)
			return buildAtomicConcept(c);
		Vector v = RacerClient.parseLispList(c);
		String constructor = (String) v.elementAt(0);
		if (constructor.equalsIgnoreCase("and"))
			return parseConjunction(v);
		if (constructor.equalsIgnoreCase("or"))
			return parseDisjunction(v);
		if (constructor.equalsIgnoreCase("some"))
			return parseSome(v);
		if (constructor.equalsIgnoreCase("all"))
			return parseAll(v);
		if (constructor.equalsIgnoreCase("at-least"))
			return parseAtLeast(v);
		if (constructor.equalsIgnoreCase("at-most"))
			return parseAtMost(v);
		if (constructor.equalsIgnoreCase("not"))
			return parseNot(v);
	...
	protected int parseInteger(String s) throws RacerIllegalConstruction {
		...
		try {
			...
			throw new RacerIllegalConstruction(s + " is not a correct integer term");
		}
		return i;
	}
	...
	public Object parseRole(String c) throws RacerIllegalConstruction {
		if (c.indexOf(' ') == -1 && c.indexOf('(') == -1 && c.indexOf(')') == -1)
			return buildAtomicRole(c);
		Vector v = RacerClient.parseLispList(c);
		String constructor = (String) v.elementAt(0);
		if (constructor.equalsIgnoreCase("inv"))
			return parseInverse(v);
