...
public class InputDateHandler implements TagHandler {
	public JComponent handleTag(HTMLPane source, final Properties attributes) {
		class Proxy extends JPanel implements TagBehavior {
			...
			public Proxy() { // assert (name != null) : "Required name attribute missing from date tag";
				...
				if (sizeAttribute != null) {
					...
					// returns garbage).
					int width = (sizeAttribute == null) ? 20 : Integer.parseInt(sizeAttribute);
	...
	private static class Test {
		public static void main(String[] args) {
			...
			pane.setText("<html>" + "<head>" + "</head>" + "<body>" + "<form>" + "<table border=1>"
					...
					+ "<tr><td> Today (date2)		</td><td align=left> <inputDate name=date2 > </td></tr>" + "</table>" + "<br><input type=submit value=Submit>" + "</form>" + "</body>" + "</html>");
