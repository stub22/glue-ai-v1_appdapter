...
abstract public class IsolatingClassLoaderBase extends URLClassLoader implements
		HRKRefinement, Ontologized.HRKAdded {
	...
	public Class loadClassRemember(String class_name, boolean resolve)
			throws ClassNotFoundException {
		Class fnd = null;
	...
	public Class loadClassParentNoResolve(String class_name)
			throws ClassNotFoundException {
		ClassLoader p = getParent();
		try {
			return PromiscuousClassUtils.callProtectedMethod(false, p,
					"loadClass", class_name, false);
	...
	public static <T> void appendURLS(final StringBuilder str, String sep,
			Iterable<T> urls) {
		boolean first = true;
		for (T u : urls) {
			Object url = u;
	...
	public static boolean pathsOf(List<Object> strings, ClassLoader cl,
			boolean includeParent) {
		if (cl == getSystemClassLoader())
			return PromiscuousClassUtils.addIfNew(strings, "$CLASSPATH");
		if (cl instanceof IsolatingClassLoaderBase) {
			return ((IsolatingClassLoaderBase) cl).addPaths(strings,
					includeParent);
		}
		if (cl instanceof URLClassLoader) {
			boolean changed = addCollection(strings, cl,
		...
		return PromiscuousClassUtils.addIfNew(strings, cl.toString());
	}
