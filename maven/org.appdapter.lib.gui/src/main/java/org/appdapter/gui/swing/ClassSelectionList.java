...
/**
 * A JavaBean-compatible GUI component consisting of list with name of the
 * classes provided in constructor and label component containing package name
 * for selected in list component class. List cells are rendered with pictures
 * corresponds with class type.
 * 
 * @author Marcin Slowik & Henrik Kniberg
 */
public class ClassSelectionList extends JJPanelList implements Chooser<Class> {
	static Logger theLogger = LoggerFactory.getLogger(ClassSelectionList.class);
	...
	/**
	 * Causes the contents of the list to be reloaded. Called automatically
	 * whenever the ancestor or package is changed
	 */
	...
	private void initComponents() {
		...
		// set new cell renderer for list control which manages appearance of
		// list
		list.setCellRenderer(new ClassListRenderer());
		// create control for displaying info about package name for selected
		// class
	 ...
	 * 
	 * @param selectedClass
	 *            New value of property selectedClass.
	 */
	 ...
	 * 
	 * @param index
	 *            class index in collection classes in list data model
	...
	private String getClassDefString(int index) {
		...
		try {
			...
			theLogger.warn("GET CLASS DEFINITION STRING EXCEPTION !!!", e);
		} finally {
			return result;
		}
	}
	 ...
	 * 
	 * @param _class
	 *            Class for which path name is needed
	 ...
	 * the list. Otherwise cell widths are computed by applying
	 * <code>getPreferredSize()</code> to the cellRenderer component for each
	 * list element.
	 * 
	 * @param width
	 ...
	 * 
	 * @param mode
	 *            an int specifying the type of selections that are permissible
	 ...
	 * 
	 * @param rowcount
	 *            an int specifying the preferred number of visible rows
	 ...
	 * 
	 * @param index
	 *            Index of class in the collection.
	 */
	private void checkControls(int index) {
		try {
			...
			theLogger.warn("CheckControl Exception", e);
	...
	/**
	 * Class used as renderer for list. It uses JLabel component as
	 * "rubber stamp" to paint the cells in the list.
	 */
	...
	/**
	 * Class defining selection listener for list box. This listener watches for
	 * changes in the list's selection and reports them to the list box.
	 */
	...
	class ClassListModel extends AbstractListModel {
		...
		/**
		 * List of values on the model list. Initialized when needed.
		 */
		java.util.List values = null;

		/**
		 * Default constructor that initialize <code>DataList</code> objects on
		 * the basis of class infos obtained by java-reflection.
		 */
		...
		/**
		 * Tells this model that a refresh is needed as soon as possible. This
		 * will be done whenever any of the display methods are called.
		 */
	...
	public static void main(String[] args) {
		try {
			...
			if (false) {
				...
				ClassBrowserPanel cbp = new ClassBrowserPanel();
				cbp.setObject(PropertyEditor.class);
				pnl = cbp;
			}
