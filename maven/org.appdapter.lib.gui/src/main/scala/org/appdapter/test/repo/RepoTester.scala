...
import com.hp.hpl.jena.sparql.modify.request.{UpdateLoad, UpdateCreate}
import com.hp.hpl.jena.sparql.sse.SSE
import com.hp.hpl.jena.update.{UpdateRequest, UpdateAction, GraphStore}
import org.appdapter.core.matdat.SheetRepo
import org.appdapter.core.log.BasicDebugger
import org.appdapter.core.name.Ident
import org.appdapter.core.store.{Repo, InitialBinding}
import org.appdapter.impl.store.{FancyRepoFactory, DatabaseRepo}

/**
 * @author Stu B. <www.texpedient.com>
 */

object RepoTesterFromAppdapter extends BasicDebugger {
  ...
  def loadGoogSheetRepo(sheetKey: String, namespaceSheetNum: Int, dirSheetNum: Int,
    ...
    val shRepo = new FabricLoaderRepo(spec, "goog:" + sheetKey + "/" + namespaceSheetNum + "/" + dirSheetNum, dirModel, fileModelCLs)
  ...
  def loadXLSXSheetRepo(sheetLocation: String, namespaceSheetName: String, dirSheetName: String,
    ...
    val shRepo = new FabricLoaderRepo(spec, "xlsx:" + sheetLocation + "/" + namespaceSheetName + "/" + dirSheetName, dirModel, fileModelCLs)
    // Load the rest of the repo's initial *sheet* models, as instructed by the directory.
    getLogger().debug("Loading Sheet Models")
    shRepo.loadSheetModelsIntoMainDataset()
    // Load the rest of the repo's initial *file/resource* models, as instructed by the directory.
