...
class DerivedRepo
  (emptyDirModel: Model, val myRepoSpec: DerivedRepoSpec) 
  extends DirectRepo(emptyDirModel, true) with RepoOper.Reloadable {
...
object DerivedRepoLoader {
  ...
  def loadPipeline(pplnGraphQN: String, repo: SheetRepo, mainDset: DataSource, myDirectoryModel: Model, fileModelCLs: java.util.List[ClassLoader]) = {
    ...
    val rc = new org.appdapter.help.repo.RepoClientImpl(repo, DefaultRepoSpecDefaultNames.DFLT_TGT_GRAPH_SPARQL_VAR, OmniLoaderRepoTest.QUERY_SOURCE_GRAPH_QN)
