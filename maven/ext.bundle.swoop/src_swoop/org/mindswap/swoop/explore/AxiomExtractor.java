				...
				for (Iterator iter = classSet.iterator(); iter.hasNext();) {
					...
					while (iterator.hasNext()) {
						...
						if (o instanceof Set && ((Set) o).contains(owlThing))
							...
							subclassAxioms.add(new ClassAxiomContainer(new OWLSubClassAxiomImpl(factory, (OWLDescription) clazz, desc), myShortform, myModel));
						}
					}
					//}
				}
			...
			for (Iterator ontIter = importClosure.iterator(); ontIter.hasNext();) {
				...
				for (Iterator classIter = ont.getClasses().iterator(); classIter.hasNext();) {
						...
						for (Iterator i = supers.iterator(); i.hasNext();) {
							...
							if (o instanceof Set && ((Set) o).contains(owlThing))
								...
								if (o instanceof Set)
									...
									desc = (OWLDescription) o;
								classAxiomContainers.add(new ClassAxiomContainer(new OWLSubClassAxiomImpl(factory, (OWLDescription) c, desc), myShortform, myModel));
							}
						}
					}
					...
					for (Iterator it = disjointsForClass.iterator(); it.hasNext();) {
						...
						classAxiomContainers.add(new ClassAxiomContainer(new OWLDisjointClassesAxiomImpl(factory, mutuallyDisjointClasses), myShortform, myModel));
					}
				...
				for (Iterator it = equivalenceMap.keySet().iterator(); it.hasNext();) {
					...
					classAxiomContainers.add(new ClassAxiomContainer(new OWLEquivalentClassesAxiomImpl(factory, equivalentClasses), myShortform, myModel));
				}
			}
