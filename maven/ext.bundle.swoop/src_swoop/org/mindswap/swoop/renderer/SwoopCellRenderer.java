		...
		if (!(value instanceof OWLEntity) && !(value instanceof OWLDataType) && !(value instanceof OWLSubClassAxiom)) {
			...
			try {
				...
				if (entity instanceof OWLIndividual) {
					...
					if (ind.isAnonymous())
						text = swoopModel.shortForm(ind.getAnonId()) + " (Anonymous)";
				}
				if (text.equals(""))
					text = swoopModel.shortForm(entity.getURI());
				...
				try {
					...
					if (rootURIs.contains(entity.getURI()))
						text = "<html>" + text + "<sup>Root</sup></html>";
				} catch (Exception e1) {
					e1.printStackTrace();
				}
