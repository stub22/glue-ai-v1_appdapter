...
public class RevertCheckpointChange extends OntologyChange {
	...
	private void parseChange() {
		...
		String scope = "Entity";
		if (change.getScope() == ChangeLog.ONTOLOGY_SCOPE)
			scope = "Ontology";
		if (change.getScope() == ChangeLog.WORKSPACE_SCOPE)
			scope = "Workspace";
		...
		if (!change.getLocationURL().equals(""))
			description += " at <b>location</b>: " + change.getLocationURL();
		description += " ]]]<br>";
	}
}
