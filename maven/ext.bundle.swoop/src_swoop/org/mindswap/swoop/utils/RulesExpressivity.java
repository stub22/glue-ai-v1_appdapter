...
public class RulesExpressivity implements SwoopModelListener, Cloneable {
	...
	public static boolean isDLAtom(OWLRuleAtom atom, OWLOntology ont) throws OWLException {
		...
		if (atom instanceof OWLRuleObjectPropertyAtom) {
			...
			if (prop.isTransitive(ont) || prop.isSymmetric(ont) || prop.isFunctional(ont))
				return true;
	...
	public void modelChanged(ModelChangeEvent event) {
		...
		if ((swModel.getEnableRules())
				&& (event.getType() == ModelChangeEvent.ONTOLOGY_LOADED || event.getType() == ModelChangeEvent.ONTOLOGY_SEL_CHANGED || event.getType() == ModelChangeEvent.ONTOLOGY_RELOADED || event
