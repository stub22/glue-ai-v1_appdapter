...
public abstract class AbstractDatasetFactory implements UserDatasetFactory {
	...
	@Override public boolean canCreateType(String typeOf, String sharedNameIgnoredPresently) {
		if (typeOf != null) {
			UserDatasetFactory tyof = RepoDatasetFactory.getUserDF_forName(typeOf);
			if (tyof == this) {
				return true;
			}
		}
		if (sharedNameIgnoredPresently != null) {
			UserDatasetFactory snip = RepoDatasetFactory.getUserDF_forName(sharedNameIgnoredPresently);
			if (snip == this) {
				return true;
			}
		}
