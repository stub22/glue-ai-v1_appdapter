...
package ext.com.hp.hpl.jena.graph.impl;
...
public class SimpleReifier implements Reifier {
	...
	final public static Map<Graph, SimpleReifier> grMap = new HashMap<Graph, SimpleReifier>();

	public static Reifier findOrCreate(Graph g, ReificationStyle style) {
		synchronized (grMap) {
			SimpleReifier queryHandler = grMap.get(g);
			if (queryHandler == null) {
				queryHandler = new SimpleReifier(g, style);
	...
	public static Reifier findOrCreate(Graph g) {
		return findOrCreate(g, ReificationStyle.Standard);
	}
	...
	*/
	public SimpleReifier(Graph parent, ReificationStyle style) {
		this(parent, new SimpleReifierTripleMap(), new SimpleReifierFragmentsMap(), style);
	}

	public SimpleReifier(Graph parent, ReifierTripleMap tm, ReifierFragmentsMap fm, ReificationStyle style) {
		this.parent = parent;
	...
	*/
	@Override public Node reifyAs(Node tag, Triple toReify) {
	...
	*/
	...
	@Override public boolean handledAdd(Triple fragment) {
		if (intercepting) {
	...
	@Override public boolean handledRemove(Triple fragment) {
		if (intercepting) {
			ReifierFragmentHandler s = fragmentsMap.getFragmentHandler(fragment);
			if (s == null)
				return false;
	...
	@Override public ExtendedIterator<Triple> find(TripleMatch m) {
		return matchesReification(m) ? tripleMap.find(m).andThen(fragmentsMap.find(m)) : Triple.None;
	}
	...
	*/
	private boolean matchesReification(TripleMatch m) {
		...
		return !predicate.isConcrete() || predicate.equals(RDF.Nodes.subject) || predicate.equals(RDF.Nodes.predicate) || predicate.equals(RDF.Nodes.object) || predicate.equals(RDF.Nodes.type)
