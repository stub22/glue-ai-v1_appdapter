...
public class SimpleReifierTripleMap implements ReifierTripleMap {
	...
	@Override public Triple getTriple(Node tag) {
	...
	*/
	...
	@Override public Triple putTriple(Node key, Triple value) {
		forwardMap.put(key, value);
		inversePut(value, key);
		return value;
	}

	@Override public void removeTriple(Node key) {
	...
	@Override public void removeTriple(Node key, Triple value) {
		forwardMap.remove(key);
		inverseRemove(value, key);
	}

	@Override public void removeTriple(Triple t) {
	...
	protected void inversePut(Triple value, Node key) {
		Set<Node> s = inverseMap.get(value);
		if (s == null)
			inverseMap.put(value, s = new HashSet<Node>());
		s.add(key);
	}

	@Override public ExtendedIterator<Node> tagIterator(Triple t) {
		Set<Node> s = inverseMap.get(t);
		return s == null ? NullIterator.<Node> instance() : WrappedIterator.create(s.iterator());
	}
	...
	*/
	@Override public ExtendedIterator<Node> tagIterator() {
