...
public class SimpleTripleSorter implements TripleSorter {
	...
	*/
	@Override public Triple[] sort(Triple[] ts) {
	...
	*/
	protected Triple[] sort() {
		while (remaining.size() > 0)
			accept(findMostBinding(findLightest(remaining)));
		return result;
	}
	...
	*/
	protected int bindingCount(Triple t, Triple other) {
		return bindingCount(t.getSubject(), other) + bindingCount(t.getPredicate(), other) + bindingCount(t.getObject(), other);
	}

	protected int bindingCount(Node n, Triple o) {
		return n.isVariable() ? bc(n, o.getSubject()) + bc(n, o.getPredicate()) + bc(n, o.getObject()) : 0;
	}
