		...
		try {
			...
			if (selTextBase == null || textLength < 1) {
				...
				if (htmlUtilities.checkParentsTag(baseTag)) {
					...
					insertHTML(parentEkit.getTextPane(), parentEkit.getExtendedHtmlDoc(), parentEkit.getTextPane().getCaretPosition(), sbNew.toString(), 0, 0, (sListType.equals("ol") ? HTML.Tag.OL
							: HTML.Tag.UL));
				}
