...
import javax.swing.text.EditorKit; // View Element ViewFactory
...
public class HTMLPane extends JTextPane {
	 ...
	 *	 that tag
	 */
	private Map tagHandlers = Collections.synchronizedMap(new HashMap());
	...
	public class HTMLPaneEditorKit extends HTMLEditorKit {
		...
		public Document createDefaultDocument() { //HTMLDocument doc = (HTMLDocument)( super.createDefaultDocument() );
		...
		protected Parser getParser() {
			...
			return new Parser() {
				public void parse(Reader r, ParserCallback callBack, boolean ignoreCharSet) throws IOException {
					p.parse(filterProvider.inputFilter(r), callBack, ignoreCharSet);
				}
			};
		}
	}
	...
	private final class CustomViewFactory extends HTMLEditorKit.HTMLFactory {
		...
		public View create(Element element) { // dumpElement( element );
			...
			if ((kind == HTML.Tag.INPUT) || (kind == HTML.Tag.SELECT) || (kind == HTML.Tag.TEXTAREA)) {
				...
				//
				FormView view = (actionListeners != null) ? new LocalFormView(element) : (FormView) (super.create(element));

				String type = (String) (element.getAttributes().getAttribute(HTML.Attribute.TYPE));
				return view;
			} else if (kind instanceof HTML.UnknownTag) { // Handling a custom element. End tags are silently ignored.
				if (element.getAttributes().getAttribute(HTML.Attribute.ENDTAG) == null) {
			...
			} else if (kind instanceof HTML.Tag) { // EkitCore handles IMG tag
	...
	private final Component doTag(Element element) {
		...
		for (Enumeration i = set.getAttributeNames(); i.hasMoreElements();) {
			...
			attributes.put((current instanceof StyleConstants) ? TAG_NAME : attributeName, attributeValue.toString());
	...
	public final class LocalFormView extends javax.swing.text.html.FormView {
		 ...
		 */
		protected void submitData(String data) { //System.out.println("Data [" + data + "]");
		...
		public void actionPerformed(ActionEvent e) {
			String type = (String) (getElement().getAttributes().getAttribute(HTML.Attribute.TYPE));
	...
	public class FormActionEvent extends ActionEvent {
		...
		private FormActionEvent(String method, String action, String name, String data) {
			...
			} catch (IOException e) { //assert false : "\"Impossible\" IOException";
			}
		}
	...
	private final void handleSubmit(final String method, final String action, final String data) {
		actionListeners.actionPerformed(new FormActionEvent(method, action, UrlUtil.decodeUrlEncoding(data) + "\n" + dataFromContributors()));
	}

	// modified
	private final void handleSubmit(final String method, final String action, final String name, final String data) {
		actionListeners.actionPerformed(new FormActionEvent(method, action, name, UrlUtil.decodeUrlEncoding(data) + "\n" + dataFromContributors()));
	}
	 ...
	 */
	public static final void addHostMapping(String thisHost, String mapsToThisLocation) {
		if (hostMap == null)
			hostMap = new HashMap();

		hostMap.put(thisHost, new HostMapping(thisHost, mapsToThisLocation));
	}
	...
	private static final class HostMapping {
		...
		public URL map(URL page) {
			try {
				...
				log.warning("Couldn't map " + remote + " to " + local + ": " + e.getMessage());
			}
			return page;
		}
	}
	...
	private final class HyperlinkHandler implements HyperlinkListener {
			...
			try {
				if (event.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
					...
					if (description.startsWith("http:") || description.startsWith("file:") || description.indexOf(":") == -1) {
						...
						if (isWindows())
							Runtime.getRuntime().exec("cmd.exe /c start " + description);
						else
							unknownRedirect(event.getDescription());
					} else {
						unknownRedirect(event.getDescription());
				...
				log.warning("Unexpected exception caught while processing hyperlink: " + e.toString() + "\n" + Log.stackTraceAsString(e));
			}
		}
		 ...
		 */
		private final void popupBrowser(URL target) throws MalformedURLException {
			...
			popupFrame.setBounds(location.x + 10, location.y + 10, size.width, size.height);
			popupFrame.show();
		}
	...
	public boolean redirect(URL page) { //assert( page.getProtocol().equals("http")
		...
		try {
			...
			String message = "HTMLPane couldn't open hyperlink: " + t.getMessage();
			log.warning(message);
			JOptionPane.showMessageDialog(HTMLPane.this, message, "401 Error", JOptionPane.WARNING_MESSAGE);
		}
		return true;
	}
	 ...
	 */
	private static final Pattern htmlExtensions = Pattern.compile("(.*\\.(html|htm|pl|jsp|asp|php|py|shtml)([?#].*)?$)" + "|(.*/[^\\./]+([?#].*)?$)" + "|(.*/([?#].*)?$)", Pattern.CASE_INSENSITIVE);
	...
	public void unknownRedirect(String request) {
		log.warning("HTMLPane: Protocol or file type not supported (" + request + ")");
		JOptionPane.showMessageDialog(HTMLPane.this, "Protocol or file type not supported (" + request + ")", "Link Error", JOptionPane.WARNING_MESSAGE);
	}
	...
	private static class Test {
		...
		public static void main(String[] args) {
			try { // Use the Windows look and feel.
				...
				mainPage = new URL(args.length >= 1 ? args[0] : "file:/c:/src/com/holub/ui/HTML/test/main.html");
				...
				pane.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent event) {
						...
						if (value != null && value.equals("Cancel")) {
							...
							System.out.println("");
							try {
								act.source().setPage(new URL("file:/c:/src/com/holub/ui/HTML/test/submit.html"));
							} catch (Exception e) {
								e.printStackTrace();
							}
							System.out.println("");
				...
				pane.addHostMapping("www.test.com", "file:/c:/src/com/holub/ui/test");
				...
				pane.addTag("size", new TagHandler() {
					public JComponent handleTag(HTMLPane source, Properties attributes) {
						source.setPreferredSize(new Dimension(Integer.parseInt(attributes.getProperty("width")), Integer.parseInt(attributes.getProperty("height"))));
						return null;
					}
				});
				...
				("holub_JLabel", new TagHandler() {
					public JComponent handleTag(HTMLPane source, Properties a) {
				...
				pane.addTag("textInput", new TagHandler() {
					public JComponent handleTag(HTMLPane source, Properties attributes) {
						return new ContributingText(attributes.getProperty("name"), attributes.getProperty("value"));
					}
				});

				pane.addTag("test", new TagHandler() {
					public JComponent handleTag(HTMLPane source, Properties attr) {
	...
	private static void createSmallFrame(final Runnable r) {
		try {
			...
			pane.setPage(new URL("file:/c:/src/com/holub/ui/HTML/test/second.html"));
