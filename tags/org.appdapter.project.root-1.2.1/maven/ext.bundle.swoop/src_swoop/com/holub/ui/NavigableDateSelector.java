...
public class NavigableDateSelector extends JPanel implements DateSelector {
	...
	private static final String NEXT_YEAR_IMAGE = "images/10px.red.arrow.right.double.gif", //{=NavigableDateSelector.NEXT_YEAR}
			NEXT_MONTH_IMAGE = "images/10px.red.arrow.right.gif", PREVIOUS_YEAR_IMAGE = "images/10px.red.arrow.left.double.gif", PREVIOUS_MONTH_IMAGE = "images/10px.red.arrow.left.gif";
	...
	private static final String FORWARD_MONTH = ">", FORWARD_YEAR = ">>", BACK_MONTH = "<", BACK_YEAR = "<<";
	...
	private final NavigationHandler navigationListener = new NavigationHandler();
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (direction == FORWARD_YEAR)
				...
				if (selector.get(Calendar.MONTH) == Calendar.DECEMBER)
					selector.roll(Calendar.YEAR, false);
			} else { // assert false:  "Unexpected direction";
			}
		}
	}
	...
	private JButton makeNavigationButton(String caption) {
		...
		URL image = (caption == FORWARD_YEAR) ? loader.getResource(NEXT_YEAR_IMAGE) : (caption == BACK_YEAR) ? loader.getResource(PREVIOUS_YEAR_IMAGE) : (caption == FORWARD_MONTH) ? loader
				.getResource(NEXT_MONTH_IMAGE) : loader.getResource(PREVIOUS_MONTH_IMAGE);

		JButton b = (image != null) ? new JButton(new ImageIcon(image)) : new JButton(caption);
	...
	public void displayDate(Calendar c) {
		selector.displayDate(c);
	}

	public void displayDate(Date d) {
		selector.displayDate(d);
	}
