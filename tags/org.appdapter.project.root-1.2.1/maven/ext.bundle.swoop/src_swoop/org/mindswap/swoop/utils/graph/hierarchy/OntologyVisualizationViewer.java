...
public class OntologyVisualizationViewer extends VisualizationViewer {
	 ...
	 */
	public OntologyVisualizationViewer(SwoopModel model, OntologyWithClassHierarchyGraph graph, Layout layout, Renderer r) {
		super(layout, r);
		myModel = model;
		myGraph = graph;
		...
		setScale((float) d.width / ld.width, (float) d.height / ld.height, new Point2D.Float());
		this.suspended = true;
	...
	protected void renderGraph() {
		...
		AlphaComposite srcOver = AlphaComposite.getInstance(AlphaComposite.SRC_OVER);
		...
		for (Iterator iter = layout.getVisibleEdges().iterator(); iter.hasNext();) {
			...
			renderer.paintEdge(offscreenG2d, e, (int) layout.getX(v1), (int) layout.getY(v1), (int) layout.getX(v2), (int) layout.getY(v2));
		}

		// paint all the vertices
		for (Iterator iter = layout.getVisibleVertices().iterator(); iter.hasNext();) {
			Vertex v = (Vertex) iter.next();
			renderer.paintVertex(offscreenG2d, v, (int) layout.getX(v), (int) layout.getY(v));
		}
	...
	public void resetHighlightedNode() {
		if (previousHighlightedNodes != null) {
			for (Iterator it = previousHighlightedNodes.iterator(); it.hasNext();) {
				ClassTreeNode previousHighlighteddNode = (ClassTreeNode) it.next();
	...
	protected final class GraphMouseImpl extends MouseAdapter implements GraphMouse {
		...
		private void handleRightClick(MouseEvent e, OntologyWithClassHierarchyRenderer rend, Point2D p, Vertex v) {
