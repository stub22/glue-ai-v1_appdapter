...
public class SwoopExpressionValidatorVisitor extends OWLObjectVisitorAdapter implements OWLValidationConstants {
	...
	public SwoopExpressionValidatorVisitor(SwoopSpeciesValidator parent, ObjectRenderer renderer) {
		this.level = LITE;
		this.parent = parent;
	...
	public void visit(OWLOr node) {
		explain(DL, UNION, "Or: " + renderNode(node));
		level = level | DL;
	}

	public void visit(OWLNot node) {
		explain(DL, COMPLEMENT, "Not: " + renderNode(node));
		level = level | DL;
	}

	public void visit(OWLEnumeration node) {
		explain(DL, ONEOF, "Enumeration: " + renderNode(node));
		level = level | DL;
	}

	public void visit(OWLDataEnumeration node) {
		explain(DL, ONEOF, /* DATARANGE?? */
				"Data Enumeration: " + renderNode(node));
		level = level | DL;
	}

	public void visit(OWLObjectValueRestriction node) {
		explain(DL, ONEOF, "Individual Value: " + renderNode(node));
		level = level | DL;
	}
	...
	public void visit(OWLObjectCardinalityRestriction node) throws OWLException {
		...
		if ((node.isAtLeast() && node.getAtLeast() > 1) || (node.isAtMost() && node.getAtMost() > 1)) {
			explain(DL, CARDINALITY, "Cardinality with > 1: " + renderNode(node));
			level = level | DL;
		}
	}
	...
	public void visit(OWLDataValueRestriction node) {
		explain(DL, ONEOF, "Data Value: " + renderNode(node));
		level = level | DL;
	}
		...
		if (topLevelDescription) {
			...
			for (Iterator it = node.getOperands().iterator(); it.hasNext();) {
				...
				if (!cori.isClassOrRestriction()) {
					...
					explain(DL, INTERSECTION, "And with non-class or restriction: " + renderNode(node));
					level = level | DL;
				} else {
			...
			explain(DL, INTERSECTION, "And: " + renderNode(node));
			level = level | DL;
		}
	}
