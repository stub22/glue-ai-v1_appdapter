...
import org.mindswap.swoop.utils.TaxoUtils;
	...
	public static void run(String[] args) throws Exception {
		...
		while (TaxonomySet.size() > 1) {
			...
			while (it.hasNext()) {
				...
				Taxonomy auxTax = TaxoUtils.merge(tax1, tax2);
		...
		while (i.hasNext()) {
			...
			auxTax = TaxoUtils.merge(fullTaxonomy, tax);
			fullTaxonomy = (Taxonomy) TaxoUtils.clone(auxTax);
		}
		...
		while (k.hasNext()) {
			...
			Taxonomy auxTax = new Taxonomy();
			auxTax = TaxoUtils.merge(fullRoleTaxonomy, tax);
			fullRoleTaxonomy = (Taxonomy) TaxoUtils.cloneTax(auxTax);
		}

		System.out.println("********");
		System.out.println("COMPLETE CLASSIFICATION TREE");
		System.out.println("********");
		TaxoUtils.printTax(fullTaxonomy);
		System.out.println("********");
		System.out.println("ROLE HIERARCHY:");
		System.out.println("********");
		TaxoUtils.printTax(fullRoleTaxonomy);
		...
		System.out.println("Number of discrepancies: " + TaxoUtils.compareTaxonomy(original, fullTaxonomy));
		...
		ObjectOutputStream outs = new ObjectOutputStream(new FileOutputStream(wkspcFile1));
		OutputFormatter output = new OutputFormatter(outs, false);
		TaxoUtils.printTax(fullTaxonomy, output);
		ObjectOutputStream outs2 = new ObjectOutputStream(new FileOutputStream(wkspcFile2));
		OutputFormatter output2 = new OutputFormatter(outs2, false);
		TaxoUtils.printTax(fullTaxonomy, output2);
