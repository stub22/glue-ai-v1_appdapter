...
public class PopupNew extends JFrame implements ActionListener, DocumentListener, KeyListener, SwoopModelListener {
	...
	private void setupUI() throws OWLException {
		...
		if (type.equals("Class"))
			parentLbl.setText("subClass-of");
		else if (type.equals("Property"))
			parentLbl.setText("subProperty-of");
		else
			parentLbl.setText("Instance-of");
		if (!type.equals("Ontology")) {
		...
		if (!type.equals("Ontology"))
			uriFld.setText(ontology.getLogicalURI().toString() + "#");
		else
			uriFld.setText("");
	...
	private void createEntity() {
		if (newEntity != null) {
			...
			if (parentBox.getSelectedIndex() != 0)
				parent = (OWLEntity) parentBox.getSelectedItem();
	...
	private void addAnnotations(OWLDataFactory df) {
		...
		try {
			...
			if (labelFld.getText() != null)
				lbl = labelFld.getText().trim();
			if (commentArea.getText() != null)
				comment = commentArea.getText().trim();
			if (idFld.getText() != null)
				version = idFld.getText().trim();
	...
	public void keyPressed(KeyEvent e) {
		...
		if ((!type.equals("Ontology")) && ((e.getSource() == idFld)) || (e.getSource() == uriFld)) {
			...
			try {
				if (!type.equals("Ontology")) {
					...
					if (swoopModel.getSelectedOntology() != null)
						thing = swoopModel.getSelectedOntology().getOWLDataFactory().getOWLClass(URI.create(OWLVocabularyAdapter.INSTANCE.getThing()));
					parentBox.addActionListener(this);
					if (this.lastSelectedParent != null) {
						if (!this.lastSelectedParent.equals(thing))
							parentBox.setSelectedItem(this.lastSelectedParent);
						else
