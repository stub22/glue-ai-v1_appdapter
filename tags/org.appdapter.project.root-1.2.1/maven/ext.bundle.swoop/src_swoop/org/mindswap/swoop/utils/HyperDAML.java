...
public class HyperDAML {
	...
	static String makeAbsolute(String maybeRelative) {
		try {
			return new java.net.URL(new java.net.URL(source), maybeRelative).toString();
	...
	static void generate(String rdf) throws ParserConfigurationException, IOException, SAXException {
		...
		org.xml.sax.helpers.DefaultHandler handler = new org.xml.sax.helpers.DefaultHandler() {
			...
			public void startElement(String namespaceURI, String localName, String rawName, org.xml.sax.Attributes atts) throws org.xml.sax.SAXException {
				if (rdfNamespace(namespaceURI) && localName.equals("RDF")) {
					rdfDepth++;
					if (!foundDAML) {
						// first time
						if (nph) {
				...
				for (int i = 0; i < atts.getLength(); i++) {
					...
					boolean hyperlink = (rdfAttribute(atts, i, "resource") || rdfAttribute(atts, i, "about"));
					boolean namespace = (attrQName.equals("xmlns") || attrQName.startsWith("xmlns:"));
			...
			public void endElement(String uri, String localName, String qName) {
				...
				if (rdfNamespace(uri) && localName.equals("RDF")) {
					rdfDepth--;
					//out.println("<br />");
			...
			void printException(String label, org.xml.sax.SAXParseException e) {
				maybeRedirect();
				startHTML();
		...
		xmlReader.setProperty("http://xml.org/sax/properties/lexical-handler", new org.xml.sax.ext.LexicalHandler() {
