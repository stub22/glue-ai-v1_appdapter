...
/**
 * This TypedResrc idea is used to associate RDF:types with references to *whole graphs* (rather than 
 * resources within graphs, *except* when such resources in turn refer to other graphs, e.g. in a "dir" model).
 */
...
object TypedResrcFactory extends BasicDebugger {
  ...
  def exposeTypedResrc(anyID: Ident, knownTypeIDs: Set[Ident], rdfNodeTrans: RdfNodeTranslator): TypedResrc = {
    anyID match {
      ...
      case otherID: Ident => {
        val jres = rdfNodeTrans.makeResourceForIdent(otherID)
