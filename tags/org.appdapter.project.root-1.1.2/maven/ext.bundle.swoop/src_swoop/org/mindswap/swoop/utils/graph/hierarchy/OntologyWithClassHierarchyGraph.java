 ...
 */
public class OntologyWithClassHierarchyGraph extends JPanel implements ActionListener, SwoopModelListener, KeyListener {
	...
	public OntologyWithClassHierarchyGraph(SwoopModel model, Vector partitions) {
		...
		this.myProps = new OntologyWithClassHierarchyGraphProperties(myModel, partitions);
	...
	public ClassTreeNode buildRootTreeNode(OWLOntology ontology) throws OWLException {
		SwoopReasoner reasoner = (SwoopReasoner) myReasonerMap.get(ontology.getURI());
	...
	public ClassTreeNode buildTreeNode(OWLOntology ontology, Set concepts, int depth) throws OWLException {
		...
		if (((ontology.isForeign(clss))) && (!clss.getURI().toString().equals(SwoopEntityRenderer.OWL_THING))) {
			return null;
		}

		ClassTreeNode root = createNode(clss, depth);

		SwoopReasoner reasoner = (SwoopReasoner) this.myReasonerMap.get(ontology.getURI());
	...
	public void populateClassList(ClassTreeNode node) {
		...
		myClassListHistoryManager.addItem(new SelectionClassListHistoryItem(this, myClassList.getModel(), node, label));
	...
	public void populateClassListWithSearchTerm(Set nodes, String subText) {
		...
		myClassListHistoryManager.addItem(new HighlightClassListHistoryItem(this, myClassList.getModel(), nodes, label));
		myCurrentClassLabel.setText(label);
		vv.repaint();
		this.myForwardListButton.setEnabled(false);
		this.myBackListButton.setEnabled(true);
	...
	protected void setupUI() {
		...
		mySplitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, false, myLeftSplitPane, gPanel);
	...
	protected JScrollPane getOntologyListPane() {
		...
		TreeSet orderedOntologySet = new TreeSet(new SwoopOntologyVertexComparator());
	...
	protected JPanel getControlPanel() {
		...
		String[] layouts = { "KK Layout", "FR Layout", "Circle Layout", "Spring Layout", "ISOM Layout", "DAG Layout" };
	...
	protected void searchForNodes(String subText) {
		...
		for (Iterator it = vertexSet.iterator(); it.hasNext();) {
			OntologyGraphNode graphNode = (OntologyGraphNode) (((Vertex) it.next()).getUserDatum(OntologyWithClassHierarchyGraph.DATA));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() instanceof JMenuItem) {
			...
			if (src == myShowSubclassAxioms) {
				...
				OntologyGraphAxiomWalker walker = new OntologyGraphAxiomWalker(picked, myModel);
				Vector subAxioms = walker.getSubclassAxioms();
				ListSelectionPopup popup = new ListSelectionPopup(this, SUBCLASS_AXIOM_LIST, subAxioms);
		...
		// controlpanel stuff
		if (cmd.equals("label")) {
			VertexStringer vs = ((JCheckBox) e.getSource()).isSelected() ? SHORT_LABEL : NO_LABEL;
			pr.setVertexStringer(vs);
		} else if (cmd.equals("inverseEdge")) {
			EdgeShapeFunction es = ((JCheckBox) e.getSource()).isSelected() ? CURVED_LINE : LINE;
	...
	public void setPartitionFocus(OWLOntology ont) {
		SwoopOntologyVertex v = (SwoopOntologyVertex) this.graph.getUserDatum(ont);
	...
	private final class VertexSize implements VertexSizeFunction {
		...
		public int getSize(Vertex vertex) {
			if (scale) {
				...
				return ((OntologyGraphNode) vertex.getUserDatum(DATA)).getDiameter();
			} else
				return MIN;
		}
	}
	...
	private class VertexLabel implements VertexStringer {
		...
		public String getLabel(Vertex vertex) {
			OntologyGraphNode node = (OntologyGraphNode) vertex.getUserDatum(DATA);
	...
	public class VertexTips implements VisualizationViewer.ToolTipListener {
		...
		public String getToolTipText(MouseEvent e) {
			...
			if (v != null) {
				OntologyWithClassHierarchyRenderer rend = (OntologyWithClassHierarchyRenderer) vv.getRenderer();
				if (rend.getIsDrawContent()) {
					...
					if (selectedNode != null) {
						...
						//vis.reset();
						//selectedNode.get
						return "<html> <b> &nbsp;" + selectedNode.toString() + "</b> <br>" + "&nbsp; &nbsp; depth: " + selectedNode.getDepth() + "</b> <br>" + "&nbsp; &nbsp; size: "
								+ (selectedNode.getSubTreeSize()) + "</b> <br>" + " </html>";
					}
				}
	...
	private final class BasicVertexColor implements VertexPaintFunction {
		...
		public Paint getDrawPaint(Vertex v) {
			return pi.isPicked(v) ? myColorScheme.getOntologyNodeSelectOutlineColor((SwoopOntologyVertex) v) : myColorScheme.getOntologyNodeOutlineColor((SwoopOntologyVertex) v);
		}

		public Paint getFillPaint(Vertex v) {
			return myColorScheme.getOntologyNodeFillColor((SwoopOntologyVertex) v);
		}
	}
