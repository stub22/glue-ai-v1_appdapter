...
public class InputNumberHandler implements TagHandler {
	public JComponent handleTag(HTMLPane source, Properties attributes) {
		class NumericTag extends NumericInput implements TagBehavior {
			...
			public NumericTag(String name, String value, String min, String max, String precision, String size) {
				super((value == null) ? 0.0 : Double.parseDouble(value), (min == null) ? -Double.MAX_VALUE : Double.parseDouble(min), (max == null) ? Double.MAX_VALUE : Double.parseDouble(max),
						(precision == null) ? 100 : Integer.parseInt(precision), BOXED, false);
		...
		JTextField control = new NumericTag(attributes.getProperty("name"), attributes.getProperty("value"), attributes.getProperty("min"), attributes.getProperty("max"),
				attributes.getProperty("precision"), attributes.getProperty("size"));
		...
		public static void main(String[] args) {
			...
			pane.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					HTMLPane.FormActionEvent event = (HTMLPane.FormActionEvent) e;
			...
			pane.setText("<html>" + "<head>" + "</head>" + "<body>" + "<form>" + "&lt;inputNumber name=\"myName\" value=\"0.0\" min=\"0\" max=\"100\" precision=\"2\" size=10&gt;" + "<table border=1>"
					+ "<tr><td> Input Number: </td><td align=left>" + "<inputNumber name=\"myName\" value=\"0.0\" min=\"0\" max=\"100\" precision=\"2\" size=10>" + "</td></tr>" + "</table>"
