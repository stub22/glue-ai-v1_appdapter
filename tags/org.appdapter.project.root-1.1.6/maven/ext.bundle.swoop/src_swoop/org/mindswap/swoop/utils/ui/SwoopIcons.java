	...
	public static Icon getIcon(Description annot) {
		try {
			if (annot.getAnnotationType() != null) {
				if (Annotea.getEntityName(annot.getAnnotationType().getURI()).equals("Comment"))
					return commentIcon;
				else if (Annotea.getEntityName(annot.getAnnotationType().getURI()).equals("SeeAlso"))
					return seeAlsoIcon;
				else if (Annotea.getEntityName(annot.getAnnotationType().getURI()).equals("Explanation"))
					return explanationIcon;
				else if (Annotea.getEntityName(annot.getAnnotationType().getURI()).equals("Question"))
					return questionIcon;
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			// entity is foreign to the reasoner's ontology
				...
				if (entity instanceof OWLObjectProperty) // Foreign object property
				{
					...
					if (prop.isLink())
						return foreignLinkPropIcon; // Foreign links are meanigless/invalid
			...
			/* ********************************************************* */
				...
				if (prop.isLink())
					return linkPropIcon;
				if (!entity.getOntologies().contains(reasoner.getOntology()))
					return importedPropIcon;
				else
					return propIcon;
			} else if (entity instanceof OWLDataProperty) {
				if (!entity.getOntologies().contains(reasoner.getOntology()))
					return importedDataPropIcon;
				else
					return dataPropIcon;
			} else if (entity instanceof OWLIndividual) {
				if (!entity.getOntologies().contains(reasoner.getOntology()))
					return importedIndividualIcon;
				else
