...
public class EConnTripleVisitor extends TripleVisitor {
	 ...
	 * @throws OWLException
	 */
	public EConnTripleVisitor(OWLOntology ontology, Model model) throws OWLException {
	...
	public void addType(OWLObject node) throws OWLException {
		if (ontology.getForeignEntities().containsKey(node)) {
			...
			add(getResource(node), getResource(owl.getForeignOntologies()), getResource(((OWLObjectProperty) node).getLinkTarget()));
		} else {
			super.addType(node);
		}
	}
