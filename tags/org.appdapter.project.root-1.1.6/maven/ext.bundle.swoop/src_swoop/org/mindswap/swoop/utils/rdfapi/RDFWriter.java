...
public class RDFWriter {
	...
	protected Map buildPropertyMap(Resource subject) throws ModelException {
		...
		for (Iterator statementIter = propModel.iterator(); statementIter.hasNext();) {
	...
	protected void serializeProperty(Resource property, RDFNode value) throws URISyntaxException, ModelException, IOException, OWLException {
		...
		if (!propertyList(value) && !propertyNested(value) && !propertyBNode(value) && !propertyReference(value) && !propertyXMLLiteral(value) && !propertyLiteral(value)) {
	...
	public void serializeSubject(Resource subject) throws IOException, OWLException {
		...
		try {
			...
			Collection types = (Collection) propertyMap.get(genResource(rdf.getInstanceOf()));
			if (types == null) {
			...
			for (Iterator typeIter = types.iterator(); typeIter.hasNext();) {
				...
				if (typeNode instanceof Resource && !SwoopResourceImpl.isAnonymous(typeNode)) {
					started = true;
			...
			for (Iterator propIter = propertyMap.keySet().iterator(); propIter.hasNext();) {
				...
				for (Iterator valueIter = values.iterator(); valueIter.hasNext();) {
					...
					if (value instanceof Literal) {
						Literal valueLit = (Literal) value;
						if (valueLit.getDatatype() == null && valueLit.getLanguage() == null && valueLit.toString().length() < 40) {

							xml.addAttribute(qnames.findPrefixURI(propertyURI), qnames.findLocal(propertyURI), valueLit.toString());
