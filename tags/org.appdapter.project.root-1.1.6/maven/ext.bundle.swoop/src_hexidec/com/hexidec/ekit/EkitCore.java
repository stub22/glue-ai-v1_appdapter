...
import com.hexidec.ekit.component.SymbolDialog;
...
public class EkitCore extends JPanel implements ActionListener, KeyListener, DocumentListener {
	...
	/** Master Constructor
	  ...
	  */
	public EkitCore(String sDocument, String sStyleSheet, String sRawDocument, StyledDocument sdocSource, URL urlStyleSheet, boolean includeToolBar, boolean showViewSource, boolean showMenuIcons,
			boolean editModeExclusive, String sLanguage, String sCountry, boolean base64, boolean debugMode, boolean hasSpellChecker, boolean multiBar) {
		super();
		...
		// javascript handling
		jtpMain = new HTMLPane();
		JavaScriptHandler jsHandler = new JavaScriptHandler();
		jtpMain.addTag("javascript", jsHandler);
		...
		jMenuFont.add(jmiMonospaced);
		JMenuItem jmiSelectFont = new JMenuItem(actionSelectFont);
		jMenuFont.add(jmiSelectFont);
		jMenuFont.addSeparator();
		...
		jMenuFormat.addSeparator();
		JMenuItem jmiClearStyles = new JMenuItem(actionClearFormat);
		if (showMenuIcons) {
			jmiClearStyles.setIcon(getEkitIcon("ClearFormat"));
		}
		;
		jMenuFormat.add(jmiClearStyles);
		...
		JMenuItem jmiSymbol = new JMenuItem(Translatrix.getTranslationString("InsertSymbol"));
		jmiSymbol.setActionCommand("insertsymbol");
		jmiSymbol.addActionListener(this);
		jMenuInsert.add(jmiSymbol);
	...
	public EkitCore(String sDocument, String sStyleSheet, String sRawDocument, URL urlStyleSheet, boolean includeToolBar, boolean showViewSource, boolean showMenuIcons, boolean editModeExclusive,
			String sLanguage, String sCountry, boolean base64, boolean debugMode, boolean hasSpellChecker, boolean multiBar) {
		this(sDocument, sStyleSheet, sRawDocument, (StyledDocument) null, urlStyleSheet, includeToolBar, showViewSource, showMenuIcons, editModeExclusive, sLanguage, sCountry, base64, debugMode,
				hasSpellChecker, multiBar);
	}

	public EkitCore(String sDocument, String sStyleSheet, String sRawDocument, URL urlStyleSheet, boolean includeToolBar, boolean showViewSource, boolean showMenuIcons, boolean editModeExclusive,
			String sLanguage, String sCountry, boolean base64, boolean debugMode) {
		this(sDocument, sStyleSheet, sRawDocument, (StyledDocument) null, urlStyleSheet, includeToolBar, showViewSource, showMenuIcons, editModeExclusive, sLanguage, sCountry, base64, debugMode,
				false, false);
	}

	public EkitCore(String sDocument, String sStyleSheet, String sRawDocument, URL urlStyleSheet, boolean showViewSource, boolean showMenuIcons, boolean editModeExclusive, String sLanguage,
			String sCountry, boolean base64, boolean debugMode, boolean hasSpellChecker) {
		this(sDocument, sStyleSheet, sRawDocument, (StyledDocument) null, urlStyleSheet, true, showViewSource, showMenuIcons, editModeExclusive, sLanguage, sCountry, base64, debugMode,
	...
	/** Common Constructor
	  ...
	  * @param sCountry          [String]  The country portion of the Internationalization Locale to run Ekit in.
	  */
	public EkitCore(String sDocument, String sStyleSheet, boolean includeToolBar, boolean showViewSource, boolean showMenuIcons, boolean editModeExclusive, String sLanguage, String sCountry,
			boolean base64) {
	...
	/** Raw/Base64 Document & Style Sheet URL Constructor (Ideal for EkitApplet)
	  ...
	  * @param sCountry          [String]  The country portion of the Internationalization Locale to run Ekit in.
	  */
	public EkitCore(String sRawDocument, URL urlStyleSheet, boolean includeToolBar, boolean showViewSource, boolean showMenuIcons, boolean editModeExclusive, String sLanguage, String sCountry,
			boolean base64, boolean hasSpellChecker, boolean multiBar) {
	...
	public void actionPerformed(ActionEvent ae) {
		try {
			...
			if (command.equals("newdoc")) {
				...
				SimpleInfoDialog sidAbout = new SimpleInfoDialog(this.getFrame(), Translatrix.getTranslationString("About"), true, Translatrix.getTranslationString("AboutMessage"),
			...
			SimpleInfoDialog sidAbout = new SimpleInfoDialog(this.getFrame(), Translatrix.getTranslationString("Error"), true, Translatrix.getTranslationString("ErrorIOException"),
					SimpleInfoDialog.ERROR);
		} catch (BadLocationException ble) {
			logException("BadLocationException in actionPerformed method", ble);
			SimpleInfoDialog sidAbout = new SimpleInfoDialog(this.getFrame(), Translatrix.getTranslationString("Error"), true, Translatrix.getTranslationString("ErrorBadLocationException"),
					SimpleInfoDialog.ERROR);
		} catch (NumberFormatException nfe) {
			logException("NumberFormatException in actionPerformed method", nfe);
			SimpleInfoDialog sidAbout = new SimpleInfoDialog(this.getFrame(), Translatrix.getTranslationString("Error"), true, Translatrix.getTranslationString("ErrorNumberFormatException"),
					SimpleInfoDialog.ERROR);
		} catch (ClassNotFoundException cnfe) {
			logException("ClassNotFound Exception in actionPerformed method", cnfe);
			SimpleInfoDialog sidAbout = new SimpleInfoDialog(this.getFrame(), Translatrix.getTranslationString("Error"), true, Translatrix.getTranslationString("ErrorClassNotFoundException "),
					SimpleInfoDialog.ERROR);
		} catch (RuntimeException re) {
			logException("RuntimeException in actionPerformed method", re);
			SimpleInfoDialog sidAbout = new SimpleInfoDialog(this.getFrame(), Translatrix.getTranslationString("Error"), true, Translatrix.getTranslationString("ErrorRuntimeException"),
					SimpleInfoDialog.ERROR);
		}
	}
	...
	/** Method for inserting a symbol
	  */
	private void insertSymbol() throws IOException, BadLocationException, RuntimeException {
		int caretPos = jtpMain.getCaretPosition();
	...
	/** Method for inserting a form element
	  */
	private void insertFormElement(HTML.Tag baseTag, String baseElement, Hashtable attribs, String[] fieldNames, String[] fieldTypes, String[] fieldValues, boolean hasClosingTag) throws IOException,
	...
	/** Alternate method call for inserting a form element
	  */
	private void insertFormElement(HTML.Tag baseTag, String baseElement, Hashtable attribs, String[] fieldNames, String[] fieldTypes, boolean hasClosingTag) throws IOException, BadLocationException,
			RuntimeException {
	...
	private int findText(String findTerm, String replaceTerm, boolean bCaseSenstive, int iOffset) {
		...
		try {
			...
			searchPlace = (bCaseSenstive ? baseDocument.getText(0, baseDocument.getLength()).indexOf(findTerm, iOffset) : baseDocument.getText(0, baseDocument.getLength()).toLowerCase()
	...
	private void insertLocalImage(File whatImage) throws IOException, BadLocationException, RuntimeException {
		...
		if (whatImage != null) {
			...
			int caretPos = jtpMain.getCaretPosition();
			htmlKit.insertHTML(htmlDoc, caretPos, "<IMG SRC=\"file:///" + whatImage + "\">", 0, 0, HTML.Tag.IMG);
			jtpMain.setCaretPosition(caretPos + 1);
			refreshOnUpdate();
		}
	}
	...
	private void insertServerImage() throws BadLocationException {
		...
		try {
			...
			//				{
			String imageURI = JOptionPane.showInputDialog(null, "Specify Image Location (Web URL):", "Insert Remote Image", JOptionPane.PLAIN_MESSAGE);
			htmlKit.insertHTML(htmlDoc, caretPos, "<IMG SRC=\"" + imageURI + "\">", 0, 0, HTML.Tag.IMG);
			//jtpMain.setCaretPosition(caretPos + 1);
			//				}
	...
	private void toggleSourceWindow() {
		if (!(isSourceWindowActive())) {
			...
			// javascript handling setText change single quote in
			// html attribute to double quote
			// e.g. name='"hello'" becomes name=""hello"" after setText		    
			// String s = jtpSource.getText();
			jtpMain.setText(jtpSource.getText());
			// String s1 = jtpMain.getText();
			iSplitPos = jspltDisplay.getDividerLocation();
