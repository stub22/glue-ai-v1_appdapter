...
public class DateUtil {
	...
	static public String supportedDateFormats(boolean asHtml) {
		Date now = new Date();
		return DateFormat.getDateInstance(DateFormat.FULL).format(now) + (asHtml ? "<br>" : " \n") + DateFormat.getDateInstance(DateFormat.LONG).format(now) + (asHtml ? "<br>" : " \n")
	...
	static public String supportedTimeFormats(boolean asHtml) {
		Date now = new Date();
		return DateFormat.getTimeInstance(DateFormat.FULL).format(now) + (asHtml ? "<br>" : " \n") + DateFormat.getTimeInstance(DateFormat.LONG).format(now) + (asHtml ? "<br>" : " \n")
	...
	static public Date parseTime(String str) {
		...
		try {
			...
			try {
				...
				try {
					date = DateFormat.getTimeInstance(DateFormat.MEDIUM).parse(str);
				} catch (ParseException pe3) {
					try {
						date = DateFormat.getTimeInstance(DateFormat.SHORT).parse(str);
					} catch (ParseException pe4) {
					}
				}
			}
