 ...
 */
public class SwoopSpeciesValidator implements org.semanticweb.owl.validation.SpeciesValidator, org.semanticweb.owl.validation.OWLValidationConstants {
		...
		try {
			...
			for (Iterator it = classURIs.iterator(); it.hasNext();) {
				...
				if (individualURIs.contains(u)) {
					...
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as Class and Individual");
				} else if (objectPropertyURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as Class and ObjectProperty");
				} else if (dataPropertyURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as Class and DataProperty");
				} else if (annotationPropertyURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as Class and AnnotationProperty");
				} else if (datatypeURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as Class and Datatype");
				}
			}
			...
			for (Iterator it = individualURIs.iterator(); it.hasNext();) {
				...
				if (objectPropertyURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as Individual and Property");
				} else if (dataPropertyURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as Individual and DataProperty");
				} else if (annotationPropertyURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as Individual and AnnotationProperty");
				} else if (datatypeURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as Individual and Datatype");
				}
			}
			...
			for (Iterator it = objectPropertyURIs.iterator(); it.hasNext();) {
				...
				if (dataPropertyURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as ObjectProperty and DataProperty");
				} else if (annotationPropertyURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as ObjectProperty and AnnotationProperty");
				} else if (datatypeURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as ObjectProperty and Datatype");
				}
			}
			...
			for (Iterator it = dataPropertyURIs.iterator(); it.hasNext();) {
				...
				if (annotationPropertyURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as DataProperty and AnnotationProperty");
				} else if (datatypeURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as DataProperty and Datatype");
				}
			}
			...
			for (Iterator it = annotationPropertyURIs.iterator(); it.hasNext();) {
				...
				if (datatypeURIs.contains(u)) {
					namespacesSeparated = false;
					explain(FULL, SEPARATIONVIOLATION, encodeHLink(u.toString(), myModel.shortForm(u)) + "\t used as AnnotationProperty and Datatype");
				}
			}
	...
	private void checkClassAxioms() throws OWLException {
		...
		for (Iterator it = allOntologies.iterator(); it.hasNext();) {
			...
			for (Iterator axit = onto.getClassAxioms().iterator(); axit.hasNext();) {
				...
				SwoopSpeciesValidatorVisitor visitor = new SwoopSpeciesValidatorVisitor(this, objectRenderer);
				try {
					oo.accept(visitor);
		...
		try {
			...
			for (Iterator it = allIndividuals.iterator(); it.hasNext();) {
				...
				if (i.getTypes(allOntologies).size() == 0) {
					...
					URI uri = i.getURI();
					explain(FULL, UNTYPEDINDIVIDUAL, "Individual with no explicit type: " + encodeHLink(uri.toString(), myModel.shortForm(uri)));
				}
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private void checkExpressivity() throws OWLException {
		try {
			...
			SwoopExpressionValidatorVisitor evv = new SwoopExpressionValidatorVisitor(this, objectRenderer);
			...
			for (Iterator it = allOntologies.iterator(); it.hasNext();) {
				...
				for (Iterator cit = onto.getClasses().iterator(); cit.hasNext();) {
					...
					if (!clazz.getEnumerations(onto).isEmpty()) {
						...
						explain(DL, ONEOF, "Enumeration used: " + encodeHLink(uri.toString(), myModel.shortForm(uri)));
					}
					...
					for (Iterator superit = clazz.getEquivalentClasses(onto).iterator(); superit.hasNext();) {
			...
			for (Iterator pit = complexProperties.iterator(); pit.hasNext();) {
				...
				for (Iterator it = allOntologies.iterator(); it.hasNext();) {
					...
					if (prop.isTransitive(onto)) {
						...
						URI uri = prop.getURI();
						explain(FULL, COMPLEXTRANSITIVE, "Complex property " + encodeHLink(uri.toString(), myModel.shortForm(uri)) + " asserted to be transitive.");
					}
				}
			}
		} catch (Exception ex) {
	...
	private void checkCorrectOWLUsage() throws OWLException {
		...
		try {
			for (Iterator it = classURIs.iterator(); it.hasNext();) {
				...
				if (uri != null && reservedVocabulary.contains(uri.toString())) {
					if (!classOnlyVocabulary.contains(uri.toString())) {
						...
						explain(FULL, BUILTINREDEFINITION, "Redefinition of: " + encodeHLink(uri.toString(), myModel.shortForm(uri)));
					}
				}
			}
			for (Iterator it = individualURIs.iterator(); it.hasNext();) {
			...
			for (Iterator it = datatypeURIs.iterator(); it.hasNext();) {
				 ...
				 */
				uri.toString().equals(OWLVocabularyAdapter.INSTANCE.getThing()) || uri.toString().equals(OWLVocabularyAdapter.INSTANCE.getNothing())) {
					/* It's a redefinition of something that we can't redefine. */
					correctOWLUsage = false;
					explain(FULL, BUILTINREDEFINITION, "Redefinition of: " + encodeHLink(uri.toString(), myModel.shortForm(uri)));
				}
		...
		try {
			for (Iterator it = allURIs.iterator(); it.hasNext();) {
				...
				if (uri != null) {
					...
					if (str.startsWith(OWLVocabularyAdapter.OWL) && !str.equals(OWLVocabularyAdapter.INSTANCE.getThing()) && !str.equals(OWLVocabularyAdapter.INSTANCE.getNothing()) &&
					...
					!OWLVocabularyAdapter.INSTANCE.getAnnotationProperties().contains(str)) {
						...
						explain(FULL, OWLNAMESPACEUSED, encodeHLink(str, myModel.shortForm(uri)) + " in OWL Namespace");
					}
				}
			}
		} catch (Exception ex) {
	...
	private String encodeHLink(String uri_string, String name) throws URISyntaxException {
		...
		return ("<a href=" + StringEscapeUtils.escapeHtml(uri_string) + ">" + StringEscapeUtils.escapeHtml(name) + "</a>");
	}
