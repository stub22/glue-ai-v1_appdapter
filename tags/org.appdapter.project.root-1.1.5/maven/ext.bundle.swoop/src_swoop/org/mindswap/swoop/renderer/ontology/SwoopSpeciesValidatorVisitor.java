...
public class SwoopSpeciesValidatorVisitor extends OWLObjectVisitorAdapter implements OWLValidationConstants {
	...
	public SwoopSpeciesValidatorVisitor(SwoopSpeciesValidator parent, ObjectRenderer renderer) {
	...
	public void visit(OWLDisjointClassesAxiom node) {
		...
		parent.explain(SwoopSpeciesValidator.DL, DISJOINT, "Disjoint Classes axiom found: " + renderNode(node));
		...
		try {
			for (Iterator it = node.getEquivalentClasses().iterator(); it.hasNext();) {
				...
				if (!isClassOrSimpleRestriction(oo)) {
					parent.explain(SwoopSpeciesValidator.DL, EXPRESSIONINAXIOM, "Equivalent Classes axiom using expressions found: " + renderNode(node));
	...
	public void visit(OWLSubClassAxiom node) {
		...
		try {
			...
			if (!isClass(subClass)) {
				parent.explain(SwoopSpeciesValidator.DL, EXPRESSIONINAXIOM, "SubClass axiom using expressions found: " + renderNode(node));
