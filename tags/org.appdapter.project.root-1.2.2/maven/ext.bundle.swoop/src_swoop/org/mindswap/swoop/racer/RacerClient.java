...
public class RacerClient extends RacerSocketClient {
	...
	public void addConceptAxiom(String tbox, String c1, String c2, boolean inc) throws java.io.IOException, RacerException {
		if (termParser != null)
			termParser.parseConcept(c1);
		if (termParser != null)
			termParser.parseConcept(c2);
	...
	public boolean alcConceptCoherent(String c, int logic) throws java.io.IOException, RacerException {
		if (termParser != null)
			termParser.parseConcept(c);
		String res = send("(alc-concept-coherent '" + c + " :logic " + (logic == RacerConstants.K_LOGIC ? ":K)" : "") + (logic == RacerConstants.K4_LOGIC ? ":K4)" : "")
	...
	public void definePrimitiveRole(String rn, int properties, String inverse, String domain, String range, String parents) throws java.io.IOException, RacerException {
		if (termParser != null)
			termParser.parseRole(rn);
		String message = "(define-primitive-role " + rn;
		if ((properties & RacerConstants.TRANSITIVE_ROLE) != 0)
			message += " :transitive t";
		if ((properties & RacerConstants.FEATURE) != 0)
			message += " :feature t";
		if ((properties & RacerConstants.SYMMETRIC_ROLE) != 0)
			message += " :symmetric t";
		if ((properties & RacerConstants.REFLEXIVE_ROLE) != 0)
			message += " :reflexive t";
		if (inverse != null)
			message += " :inverse " + inverse;
		if (domain != null)
	...
	protected String[] getResultAndWarningFromAnswer(String result) {
		...
		if (fi + 4 < result.length())
			s[1] = result.substring(fi + 3, result.length() - 1);
		return s;
	}
	...
	protected String getWarningFromOK(String result) {
		...
		if (ini < fi - 1)
			warning = result.substring(ini + 1, fi);
		return warning;
	}
	...
	public String cloneABox(String abn, String newabn, boolean overwrite) throws RacerException, IOException {
		String res = send("(clone-abox " + abn + " :new-name " + newabn + " :overwrite " + (overwrite ? "t" : "nil") + ")", true);
		return res;
	}
	...
	public String cloneTBox(String tbn, String newtbn, boolean overwrite) throws RacerException, IOException {
		String res = send("(clone-tbox " + tbn + " :new-name " + newtbn + " :overwrite " + (overwrite ? "t" : "nil") + ")", true);
		return res;
	}
	...
	protected static String parseConcept(String concept) {
		if (concept.equalsIgnoreCase("*TOP*"))
			concept = "TOP";
		else if (concept.equalsIgnoreCase("*BOTTOM*"))
			concept = "BOTTOM";
		return concept;
	}
	...
	protected static ConceptAssertion[] parseConceptAssertionList(String res) {
		if (res.equalsIgnoreCase("nil"))
			return new ConceptAssertion[0];
	...
	protected static Vector parseLispList(String s) {
		...
		for (posIni = 1; s.charAt(posIni) == ' ' || s.charAt(posIni) == 13 || s.charAt(posIni) == '\t' || s.charAt(posIni) == 10; posIni++)
			;
		for (;;) {
			posFi = posIni;
			for (; posFi < s.length() && (np > 1 || (s.charAt(posFi) != ' ' && s.charAt(posFi) != 13 && s.charAt(posFi) != '\t' && s.charAt(posFi) != 10) || strictSymbol || string); posFi++) {
				if (strictSymbol && s.charAt(posFi) == '|')
					strictSymbol = false;
				else if (string && s.charAt(posFi) == '"')
					string = false;
				else if (!string && s.charAt(posFi) == '|')
					strictSymbol = true;
				else if (!strictSymbol && s.charAt(posFi) == '"')
					string = true;
				else if (!strictSymbol && !string && s.charAt(posFi) == '(')
					np++;
				else if (!strictSymbol && !string && s.charAt(posFi) == ')')
					np--;
			}
			...
			// Es salta els blancs
			for (posIni = posFi + 1; s.charAt(posIni) == ' ' || s.charAt(posIni) == 13 || s.charAt(posIni) == '\t' || s.charAt(posIni) == 10; posIni++)
				;
		}
	...
	public boolean roleSubsumesP(String r1, String r2) throws java.io.IOException, RacerException {
		if (termParser != null)
			termParser.parseRole(r1);
		if (termParser != null)
			termParser.parseRole(r2);
	...
	public void signature(String concepts, String roles, String individuals) throws RacerException, IOException {
		send("(signature " + (concepts == null ? "" : " :atomic-concepts " + concepts) + (roles == null ? "" : " :roles " + roles) + (individuals == null ? "" : " :individuals " + individuals) + ")",
				false);
	}
