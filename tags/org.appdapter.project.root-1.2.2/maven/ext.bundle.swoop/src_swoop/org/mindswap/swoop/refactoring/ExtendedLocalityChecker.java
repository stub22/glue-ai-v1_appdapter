	...
	public boolean isLocal(OWLDescription desc) throws OWLException {
		if (desc instanceof OWLClass) {
			...
			if (desc instanceof OWLObjectSomeRestriction) {
				if (DEBUG) {
					System.out.println("The SomeValuesRestriction on Role" + ((OWLObjectSomeRestriction) desc).getProperty().getURI().toString() + "is local");
				}
				return true;
			}
	...
	public Set getNonLocalAxioms() throws OWLException {
		...
		while (iter.hasNext()) {
			...
			if (axiom instanceof OWLSubClassAxiom) {
				if (!(this.isLocal(((OWLSubClassAxiom) axiom).getSubClass())) && this.isLocal(((OWLSubClassAxiom) axiom).getSuperClass())) {
