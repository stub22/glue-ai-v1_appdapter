...
public class DateSelectorPanel extends JPanel implements DateSelector {
	...
	static { // Initialize weekdays and months with Local-specific
			 // strings.
	...
	private JButton[] dayNames = { newDayButton(weekdays[1].charAt(0)), newDayButton(weekdays[2].charAt(0)), newDayButton(weekdays[3].charAt(0)), newDayButton(weekdays[4].charAt(0)),
			newDayButton(weekdays[5].charAt(0)), newDayButton(weekdays[6].charAt(0)), newDayButton(weekdays[7].charAt(0)) };
	...
	public DateSelectorPanel() //{=DateSelectorPanel.noarg}
	{
		...
		calendarDisplay.setLayout(new GridLayout(MAX_WEEKS + 1 /*rows*/, DAYS_IN_WEEK /*columns*/));
		for (int i = 0; i < dayNames.length; ++i)
			//{=DateSelectorPanel.init.grid}
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getActionCommand().equals("D")) {
				...
				if (text.length() > 0) //  <=0 means click on blank square. Ignore.
				{
					...
					fire_ActionEvent(SELECT_ACTION, calendar.getTime().toString());
				}
			}
		}
	}
	...
	private void updateCalendarDisplay() {
		...
		for (; i < days.length; ++i) /* 3 */
		{
			...
			if (calendar.get(Calendar.MONTH) == today.get(Calendar.MONTH) && calendar.get(Calendar.YEAR) == today.get(Calendar.YEAR) && calendar.get(Calendar.DATE) == today.get(Calendar.DATE)) {
				highlight(days[i]);
			}
	...
	private static class Test {
		public static void main(String[] args) {
			...
			((NavigableDateSelector) center).changeNavigationBarColor(Colors.TRANSPARENT);
