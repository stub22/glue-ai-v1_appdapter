...
public final class PatternStageCompiler implements PatternCompiler {
	...
	*/
	public static Pattern[] compile(PatternCompiler compiler, Mapping map, Triple[] source) {
		...
		for (int i = 0; i < source.length; i += 1)
			compiled[i] = compile(compiler, source[i], map);
		return compiled;
	}
	...
	*/
	@Override public Element fixed(Node value) {
		return new Fixed(value);
	}

	@Override public Element bound(Node n, int index) {
		return new Bound(index);
	}

	@Override public Element bind(Node n, int index) {
		return new Bind(index);
	}

	@Override public Element any() {
		return Element.ANY;
	}
}
