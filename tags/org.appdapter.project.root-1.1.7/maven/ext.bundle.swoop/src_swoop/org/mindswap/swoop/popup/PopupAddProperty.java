	...
	private void addPropertyChange() {
		...
		try {
			...
			if ((newPropFld.getText() != null) && (!newPropFld.getText().trim().equals(""))) {
				...
				// **create new property
				if (propTypeBox.getSelectedIndex() == 0)
					prop = swoopModel.getSelectedOntology().getOWLDataFactory().getOWLDataProperty(propURI);
				else
					prop = swoopModel.getSelectedOntology().getOWLDataFactory().getOWLObjectProperty(propURI);
				makeChange(prop);
			} else {
		...
		case 5: // add inverse
			...
			if (prop instanceof OWLDataProperty) {
				...
				if (!propSubj.isLink()) {
					...
					try {
						OntologyChange oc = new AddForeignEntity(dispOnt, prop, dispOnt.getURI(), null);
						changes.add(oc);
					} catch (OWLException e) {
					}
					...
					OntologyChange oc2 = new RemoveEntity(dispOnt, prop, null);
					changes.add(oc2);
