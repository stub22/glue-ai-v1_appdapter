	...
	TreeCellRenderer treeCellRenderer = new DefaultTreeCellRenderer() {
		public Component getTreeCellRendererComponent(JTree tree, Object value, boolean sel, boolean expanded, boolean leaf, int row, boolean hasFocus) {
			...
			if (value instanceof TreeTableNode) {
				...
				if (node.swoopChange.isRedundant)
					setIcon(icons.questionIcon);
				else if (node.swoopChange.isTopNode)
					setIcon(icons.rootIcon);
				else if (node.swoopChange.isOnRepository)
					setIcon(icons.explanationIcon);
				else
					setIcon(icons.commentIcon);
			}
	...
	public class TreeTableCellRenderer extends JTree implements TableCellRenderer {
		...
		public void paint(Graphics g) {
			...
			if (highlightBorder != null) {
				highlightBorder.paintBorder(this, g, 0, visibleRow * getRowHeight() - 1, getWidth(), getRowHeight() + 1);
			}
		}
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
			...
			} else if (table.getShowHorizontalLines() || table.getShowVerticalLines()) {
				...
				if (me.getModifiers() == 0 || me.getModifiers() == InputEvent.BUTTON1_MASK) {
					for (int counter = getColumnCount() - 1; counter >= 0; counter--) {
						if (getColumnClass(counter) == TreeTableModel.class) {
							MouseEvent newME = new MouseEvent(JTreeTable.this.tree, me.getID(), me.getWhen(), me.getModifiers(), me.getX() - getCellRect(0, counter, true).x, me.getY(),
