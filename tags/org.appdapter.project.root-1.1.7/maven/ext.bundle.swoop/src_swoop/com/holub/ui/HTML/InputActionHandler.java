...
public class InputActionHandler implements TagHandler {
	...
	InputActionHandler(HTMLPane pane) {
		this.pane = pane;
	}
	...
	public JComponent handleTag(HTMLPane source, Properties attributes) {
		...
		class ButtonTag extends JButton implements TagBehavior {
			...
			public String getFormData() {
				...
				return (name == null) ? "" : (name + "=" + pressed[0]);
			}
		}
	...
	private static class Test {
		public static void main(String[] args) {
			...
			pane.setText("<html>" + "<head>" + "</head>" + "<body>" + "&lt;inputAction name=myName value=\"initialValue\" &gt;" + "<form>" + "<table border=1>"
					+ "<tr><td> Input Action: </td><td align=left> <inputAction name=myName value=\"initialValue\" > </td></tr>" + "</table>" + "<br><input type=submit value=Submit>" + "</form>"
					+ "</body>" + "</html>");
