...
public class UrlUtil {
	...
	public static URL relative(URL context, String spec) throws MalformedURLException {
		...
		*/
		...
		while (spec.startsWith("..", specStart)) {
			...
			if (pathEnd == -1)
				throw new MalformedURLException("Illegal relative path. Cannot get to " + originalSpec + " from " + originalPath);

			if (spec.startsWith("/", specStart))
				++specStart;
		}

		return new URL(context.getProtocol() + "://" + context.getHost() + path.substring(0, pathEnd + 1) + spec.substring(specStart));
	}
	...
	public static String decodeUrlEncoding(String data) {
		...
		for (int cursor = 0; cursor < data.length(); ++cursor) {
			...
			case '%':
				...
				high = (high >= 'a') ? (high - 'a') + 0xa : (high >= 'A') ? (high - 'A') + 0xa : (high - '0');
				low = (low >= 'a') ? (low - 'a') + 0xa : (low >= 'A') ? (low - 'A') + 0xa : (low - '0');
				c = (high << 4) + low;
